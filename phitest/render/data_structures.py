...
from lib.util import is_None_or_type, load_numpy

from .camera import Camera
from .renderer import Renderer
from .transform import GridTransform, Transform
...
def get_coord_field(shape, offset=[0, 0, 0], lod=0.0, concat=True):
    ...
    """
    coord_z, coord_y, coord_x = tf.meshgrid(
    ...
    coord_data = [
        tf.reshape(coord_x + offset[0], [1] + shape + [1]),
        tf.reshape(coord_y + offset[1], [1] + shape + [1]),
        tf.reshape(coord_z + offset[2], [1] + shape + [1]),
    ]  # 3 x 1DHW1
    if lod is not None:
...
class ImageSet:
    def __init__(
        self, base_images, shape=None, device=None, var_name="image_set", trainable=True, resize_method="LINEAR"
    ):
        ...
        assert isinstance(
            base_images, (tf.Tensor, tf.Variable, np.ndarray)
        ), "ImageSet: images must be tf.Tensor or np.ndarray."
        ...
        elif not image_rank == 5:
            raise ValueError("ImageSet: images must be rank 5, NVHWC.")
    ...
    def _create_size(self, shape):
        ...
        if not (has_shape(shape, [2])):
            raise ValueError(
                "ImageSet.resize(): image shape must be (height, width), is %s with shape %s"
        ...
        if np.all(self.base_shape.yx.value == shape):
            ...
            images = tf.reshape(
                self.base_images, (base_shape.n * base_shape.z, base_shape.y, base_shape.x, base_shape.c)
            )
            if self.resize_method == "LINEAR":
                ...
                images = tf_image_resize_mip(
                    images, shape, mip_bias=float("-inf"), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR
                )
    ...
    def save(self, renderer, path, format="PNG", idx=0, name=None):
        if name is None:
            name = self.var_name
        shape = self.shape
        renderer.write_images_batch_views(
            self.base_images,
            "%s_b{batch:04d}_v{view:02d}_{idx:04d}" % name,
            base_path=path,
            frame_idx=idx,
            input_format="NVHWC",
            image_format=format,
    ...
    def save_MS_stack(self, renderer, path, format="PNG", idx=0, name=None):
        ...
        for scale, image in self._MS_images.items():
            renderer.write_images_batch_views(
                image,
                "%s_%02d_b{batch:04d}_v{view:02d}_{idx:04d}" % (name, scale),
                base_path=path,
                frame_idx=idx,
                input_format="NVHWC",
                image_format=format,
...
class ImageSetMS(ImageSet):
    def __init__(self, base_images, device=None, var_name="image_set_MS", trainable=True, resize_method="LINEAR"):
        ...
        for s in self._scales_MS:
            ...
            assert isinstance(
                scale_image, (tf.Tensor, tf.Variable, np.ndarray)
            ), "ImageSet: images must be tf.Tensor or np.ndarray."
            ...
            elif not scale_rank == 5:
                raise ValueError("ImageSet: images [%d] must be rank 4 or 5, NVHWC." % (s))
        ...
        super().__init__(
            self._base_images_MS[self._scales_MS[-1]],
            shape=None,
            device=device,
            var_name=var_name,
            trainable=trainable,
    ...
    def _check_base_MS_scale(self, scale):
        if not self.has_base_MS_scale(scale):
            raise KeyError(
                "Scale {} not in ImageSetMS '{}', available scales: {}".format(scale, self.var_name, self._scales_MS)
            )
    ...
    def save_base_MS_stack(self, renderer, path, format="PNG", idx=0, name=None):
        ...
        for scale, image in self._base_images_MS.items():
            renderer.write_images_batch_views(
                image,
                "%s_MS%02d_b{batch:04d}_v{view:02d}_{idx:04d}" % (name, scale),
                base_path=path,
                frame_idx=idx,
                input_format="NVHWC",
                image_format=format,
...
class Zeroset:
    ...
    def _hull_staggered_lerp_weight(self, a, b):
        ...
        return tf.where(
            tf.logical_xor(a_leq, tf.less_equal(b, 0)),  # sign change along iterpolation
    ...
    def to_hull_simple_staggered(self):
        return (
            self._hull_simple_staggered_component(-2),
            self._hull_simple_staggered_component(-3),
            self._hull_simple_staggered_component(-4),
        )
...
class DensityGrid:
    def __init__(
        ...
        assert isinstance(restrict_to_hull, bool), "DensityGrid: restrict_to_hull must be bool, is: %s" % (
            type(restrict_to_hull).__name__,
        )
        ...
        if as_var:
            ...
            with tf.device(self._device):
                self._d = tf.Variable(
                    initial_value=d if d is not None else rand_init(shape=[1] + self.shape + [1], dtype=tf.float32),
                    name=var_name + "_dens",
                    trainable=True,
                )
        ...
        if inflow is not None:
            ...
            self._inflow_padding = (
                [[0, 0]]
                + [[inflow_offset[_], self.shape[_] - inflow_offset[_] - inflow_shape[_]] for _ in range(3)]
                + [[0, 0]]
            )
        ...
        try:
            with np.load(path) as np_data:
                ...
                grid = cls(
                    ...
                    hull=hull,
                    inflow=inflow,
                    inflow_offset=inflow_offset,
                    inflow_mask=inflow_mask,
                    device=device,
                    var_name=var_name,
                    trainable=trainable,
                    restrict_to_hull=restrict_to_hull,
                    is_SDF=is_SDF,
                )
        except:
    ...
    def copy(self, as_var=None, device=None, var_name=None, trainable=None, restrict_to_hull=None):
        ...
        if self._inflow is not None:
            grid = DensityGrid(
                self.shape,
                d=tf.identity(self._d),
                as_var=as_var,
                scale_renderer=self.scale_renderer,
                hull=self.hull,
                inflow=tf.identity(self._inflow),
                inflow_offset=self.inflow_offset,
    ...
    def rescale(self, new_shape, base_scale_fn):
        ...
        hull = (
            self.scale_renderer.resample_grid3D_aligned(self.base_hull, new_shape) if self.hull is not None else None
        )
        if self._inflow is not None:
            ...
            if_mask = (
                ...
                else self.scale_renderer.resample_grid3D_aligned(self.base_inflow_mask, if_shape)
            )
        ...
        with tf.device(self._device):
            ...
            if self._inflow is not None:
                ...
                inflow_shape = spacial_shape_list(self._inflow)  # .get_shape().as_list()[-4:-1]
                self._inflow_padding = (
                    [[0, 0]] + [[if_off[_], self.shape[_] - if_off[_] - inflow_shape[_]] for _ in range(3)] + [[0, 0]]
                )
    ...
    def apply_clamp(self, vmin, vmax):
        ...
        if self._inflow is not None:
            ...
            density_cropped = d[
                self._inflow_padding[0][0] : denstiy_shape[0] - self._inflow_padding[0][1],
    ...
    def assign(self, d, inflow=None):
        ...
        if not len(shape) == 5 or not shape[-1] == 1 or not shape[-4:-1] == self.shape:
            raise ValueError(
                "Invalid or incompatible shape of density on assignment: is {}, required: NDHW1 with DHW={}".format(
                    shape, self.shape
                )
            )
        if self.is_var:
    ...
    def stats(self, mask=None, state=None, **warp_kwargs):
        ...
        if (
            ...
            and state.prev.density is not None
            and state.prev.velocity is not None
        ):
...
class VelocityGrid:
    ...
    def __init__(
        ...
        *,
        ...
        if as_var:
            ...
            with tf.device(self._device):
                self._x = tf.Variable(
                    initial_value=x if x is not None else rand_init(shape=[1] + self.x_shape + [1], dtype=tf.float32),
                    name=var_name + "_x",
                    trainable=True,
                )
                self._y = tf.Variable(
                    initial_value=y if y is not None else rand_init(shape=[1] + self.y_shape + [1], dtype=tf.float32),
                    name=var_name + "_y",
                    trainable=True,
                )
                self._z = tf.Variable(
                    initial_value=z if z is not None else rand_init(shape=[1] + self.z_shape + [1], dtype=tf.float32),
                    name=var_name + "_z",
                    trainable=True,
                )
        else:
        ...
        self.scale_renderer = (
            scale_renderer  # Renderer(profiler, filter_mode='LINEAR', mipmapping='NONE', sample_gradients=False)
        )
        if self.scale_renderer is not None:
            if (self.outer_bounds == "CLOSED" and self.scale_renderer.boundary_mode != "BORDER") or (
                self.outer_bounds == "OPEN" and self.scale_renderer.boundary_mode != "CLAMP"
            ):
                LOG.warning(
                    "Velocity outer boundary %s does not match scale renderer boundary mode %s",
                    self.outer_bounds,
                    self.scale_renderer.boundary_mode,
                )
        self.warp_renderer = warp_renderer
        if self.warp_renderer is not None:
            if (self.outer_bounds == "CLOSED" and self.warp_renderer.boundary_mode != "BORDER") or (
                self.outer_bounds == "OPEN" and self.warp_renderer.boundary_mode != "CLAMP"
            ):
                LOG.warning(
                    "Velocity outer boundary %s does not match scale renderer boundary mode %s",
                    self.outer_bounds,
                    self.warp_renderer.boundary_mode,
                )
    ...
    def from_file(
        ...
        try:
            with np.load(path) as vel:
                ...
                vel_grid = cls(
                    shape,
                    x=vel["vel_x"].astype(np.float32),
                    y=vel["vel_y"].astype(np.float32),
                    z=vel["vel_z"].astype(np.float32),
                    as_var=as_var,
                    boundary=boundary,
                    scale_renderer=scale_renderer,
                    warp_renderer=warp_renderer,
    ...
    def from_scalarFlow_file(
        ...
        v_z = tf.pad(
            -v_z, [p0, (1, 0), p0, p0, p0], "SYMMETRIC"
        )  # z value/direction reversed, pad lower value as axis is reversed (?)
    ...
    def copy(self, as_var=None, device=None, var_name=None, trainable=None):
        ...
        grid = VelocityGrid(
            self.centered_shape,
            x=tf.identity(self._x),
            y=tf.identity(self._y),
            z=tf.identity(self._z),
            as_var=as_var,
            boundary=self.boundary,
            scale_renderer=self.scale_renderer,
            warp_renderer=self.warp_renderer,
            device=device,
            var_name=var_name,
            trainable=trainable,
        )
    ...
    def resample_velocity(scale_renderer, vel, scale=None, shape=None, is_staggered=True, scale_magnitude=True):
        if scale is None and shape is None:
            raise ValueError("You must provide 1 of scale or shape.")
            ...
            if isinstance(vel, tuple):
                ...
                vel_components = [
                    scale_renderer.resample_grid3D_aligned(
                        x, x_shape, align_x="CENTER", align_y="BORDER", align_z="BORDER"
                    ),
                    scale_renderer.resample_grid3D_aligned(
                        y, y_shape, align_x="BORDER", align_y="CENTER", align_z="BORDER"
                    ),
                    scale_renderer.resample_grid3D_aligned(
                        z, z_shape, align_x="BORDER", align_y="BORDER", align_z="CENTER"
                    ),
                ]
                ...
                vel_components = [
                    scale_renderer.resample_grid3D_aligned(
                        x, shape, align_x="CENTER", align_y="BORDER", align_z="BORDER"
                    ),
                    scale_renderer.resample_grid3D_aligned(
                        y, shape, align_x="BORDER", align_y="CENTER", align_z="BORDER"
                    ),
                    scale_renderer.resample_grid3D_aligned(
                        z, shape, align_x="BORDER", align_y="BORDER", align_z="CENTER"
                    ),
                ]
    ...
    def copy_scaled(
        self, centered_shape, scale_magnitude=True, as_var=None, device=None, var_name=None, trainable=None
    ):
        if as_var is None:
            as_var = self.is_var
        if as_var and var_name is None:
    ...
    def copy_warped(
        self, vel_grid=None, as_var=None, order=1, dt=1.0, device=None, var_name=None, clamp="NONE", trainable=None
    ):
        if as_var is None:
            as_var = self.is_var
        if as_var and var_name is None:
    ...
    def get_output_variables(
        self, centered=True, staggered=True, include_MS=False, include_residual=False, only_trainable=False
    ):
        return {"velocity_x": self._x, "velocity_y": self._y, "velocity_z": self._z}

    def save(self, path):
        np.savez_compressed(
            path, centered_shape=self.centered_shape, vel_x=self.x.numpy(), vel_y=self.y.numpy(), vel_z=self.z.numpy()
        )
    ...
    def assign(self, x, y, z):
        ...
        if not len(x_shape) == 5 or not x_shape[-1] == 1 or not x_shape[-4:-1] == self.x_shape:
            raise ValueError(
                "Invalid or incompatible shape of velocity x component on assignment: is {}, required: NDHW1 with DHW={}".format(
        ...
        if not len(y_shape) == 5 or not y_shape[-1] == 1 or not y_shape[-4:-1] == self.y_shape:
            raise ValueError(
                "Invalid or incompatible shape of velocity y component on assignment: is {}, required: NDHW1 with DHW={}".format(
        ...
        if not len(z_shape) == 5 or not z_shape[-1] == 1 or not z_shape[-4:-1] == self.z_shape:
            raise ValueError(
                "Invalid or incompatible shape of velocity z component on assignment: is {}, required: NDHW1 with DHW={}".format(
    ...
    def assign_centered(self, centered_grid):
        ...
        centered_shape = shape_list(centered_grid)
        if (
            not len(centered_shape) == 5
            or not centered_shape[-1] == 3
            or not centered_shape[-4:-1] == self.centered_shape
        ):
            raise ValueError(
                "Invalid or incompatible shape of centered velocity assignment: is {}, required: NDHW3 with DHW={}".format(
                    centered_shape, self.centered_shape
                )
            )
    ...
    def assign_centered_scaled(self, centered_grid, scale_magnitude=True):
        ...
        x, y, z = self._centered_to_staggered(
            centered_grid, output_centered_shape=self.centered_shape, scale_magnitude=scale_magnitude
        )
        if self.is_var:
            self._x.assign(x)
            self._y.assign(y)
    ...
    def _centered_to_staggered_old(self, centered):
        ...
        with self.scale_renderer.profiler.sample("centered velocity to staggered"):
            ...
            centered_x_transform = GridTransform(
                self.centered_shape, scale=[2.0 / _ for _ in self.x_shape[::-1]], center=True
            )
            centered_y_transform = GridTransform(
                self.centered_shape, scale=[2.0 / _ for _ in self.y_shape[::-1]], center=True
            )
            centered_z_transform = GridTransform(
                self.centered_shape, scale=[2.0 / _ for _ in self.z_shape[::-1]], center=True
            )
            ...
            x = tf.squeeze(
                self.scale_renderer._sample_transform(x, [centered_x_transform] * batch_size, [staggered_x_transform]),
                1,
            )
            y = tf.squeeze(
                self.scale_renderer._sample_transform(y, [centered_y_transform] * batch_size, [staggered_y_transform]),
                1,
            )
            z = tf.squeeze(
                self.scale_renderer._sample_transform(z, [centered_z_transform] * batch_size, [staggered_z_transform]),
                1,
            )
        return x, y, z
    ...
    def _centered_to_staggered(self, centered, output_centered_shape=None, scale_magnitude=True):
        ...
        with self.scale_renderer.profiler.sample("centered velocity to staggered"):
            ...
            x, y, z = tf.split(centered, 3, axis=-1)
            x = self.scale_renderer.resample_grid3D_aligned(
                x, x_shape, align_x="CENTER", align_y="BORDER", align_z="BORDER"
            )
            y = self.scale_renderer.resample_grid3D_aligned(
                y, y_shape, align_x="BORDER", align_y="CENTER", align_z="BORDER"
            )
            z = self.scale_renderer.resample_grid3D_aligned(
                z, z_shape, align_x="BORDER", align_y="BORDER", align_z="CENTER"
            )
    ...
    def _scalar_centered_to_staggered(self, centered, *, allow_split_channels=False):
        ...
        with self.scale_renderer.profiler.sample("centered scalar to staggered"):
            if allow_split_channels and centered_shape[-1] not in [1, 2, 4]:
                ...
                for split in splits:
                    staggered.append(
                        self.scale_renderer.resample_grid3D_aligned(
                            split,
                            staggered_shape,
                            align_x="STAGGER_OUTPUT",
                            align_y="STAGGER_OUTPUT",
                            align_z="STAGGER_OUTPUT",
                        )
                    )
                staggered = tf.concat(staggered, axis=-1)
                assert shape_list(staggered)[-1] == centered_shape[-1]
            else:
                staggered = self.scale_renderer.resample_grid3D_aligned(
                    centered,
                    staggered_shape,
                    align_x="STAGGER_OUTPUT",
                    align_y="STAGGER_OUTPUT",
    ...
    def _sampled_to_shape(self, shape):
        with self.scale_renderer.profiler.sample("velocity to shape"):
            ...
            vel_sampled = [
                (
                    tf.squeeze(
                        self.scale_renderer._sample_transform(self.x, [staggered_x_transform], [sample_transform]), 1
                    )
                    if not shape == self.x_shape
                    else tf.identity(self.x)
                ),  # 1DHW1
                (
                    tf.squeeze(
                        self.scale_renderer._sample_transform(self.y, [staggered_y_transform], [sample_transform]), 1
                    )
                    if not shape == self.y_shape
                    else tf.identity(self.y)
                ),
                (
                    tf.squeeze(
                        self.scale_renderer._sample_transform(self.z, [staggered_z_transform], [sample_transform]), 1
                    )
                    if not shape == self.z_shape
                    else tf.identity(self.z)
                ),
            ]
        return vel_sampled
    ...
    def _check_vel_component_shape(x, y, z):
        ...
        #   z: (+1,+0,+0)
        x_shape = shape_list(x)
        if len(x_shape) != 5 or x_shape[-1] != 1:
            return False
        ...
        y_shape = shape_list(y)
        if len(y_shape) != 5 or y_shape[-1] != 1:
            return False
        if y_shape[-4] != c_shape[0] or y_shape[-3] != (c_shape[1] + 1) or y_shape[-2] != c_shape[2]:
            return False
        if y_shape[-1] != batch:
            return False

        z_shape = shape_list(z)
        if len(z_shape) != 5 or z_shape[-1] != 1:
            return False
        if z_shape[-4] != (c_shape[0] + 1) or z_shape[-3] != c_shape[1] or z_shape[-2] != c_shape[2]:
            return False
        if z_shape[-1] != batch:
            return False
    ...
    def _check_potential_component_shape(x, y, z):
        ...
        if len(y_shape) != 5 or y_shape[-1] != 1:
            return False
        if y_shape[-4] != (c_shape[0] + 1) or y_shape[-3] != (c_shape[1]) or y_shape[-2] != (c_shape[2] + 1):
            return False
        if y_shape[-1] != batch:
            return False
        ...
        if len(z_shape) != 5 or z_shape[-1] != 1:
            return False
        if z_shape[-4] != (c_shape[0]) or z_shape[-3] != (c_shape[1] + 1) or z_shape[-2] != (c_shape[2] + 1):
            return False
        if z_shape[-1] != batch:
            return False
    ...
    def _components_to_centered(self, x, y, z, concat=True):
        if not self._check_vel_component_shape(x, y, z):
            raise ValueError(
                "shapes of components do not fit. x: {}, y: {}, z: {}".format(
                    shape_list(x), shape_list(y), shape_list(z)
                )
            )
    ...
    def _staggered_components_potential_to_staggered_components(self, pot_x, pot_y, pot_z):
        if not self._check_potential_component_shape(pot_x, pot_y, pot_z):
            raise ValueError(
                "shapes of potential components do not fit. x: {}, y: {}, z: {}".format(
                    shape_list(pot_x), shape_list(pot_y), shape_list(pot_z)
                )
            )

        vel_components = [
            (pot_z[:, :, :-1, :, :] - pot_z[:, :, 1:, :, :])
            - (pot_y[:, :-1, :, :, :] - pot_y[:, 1:, :, :, :]),  # dPz/dy - dPy/dz
            (pot_x[:, :-1, :, :, :] - pot_x[:, 1:, :, :, :])
            - (pot_z[:, :, :, :-1, :] - pot_z[:, :, :, 1:, :]),  # dPx/dz - dPz/dx
            (pot_y[:, :, :, :-1, :] - pot_y[:, :, :, 1:, :])
            - (pot_x[:, :, :-1, :, :] - pot_x[:, :, 1:, :, :]),  # dPy/dx - dPx/dy
        ]
    ...
    def _staggeredTensor_potential_to_staggered_components(self, pot):
        ...
        vel_components = [
            (pot_z[:, :-1, :-1, :, :] - pot_z[:, :-1, 1:, :, :])
            - (pot_y[:, :-1, :-1, :, :] - pot_y[:, 1:, :-1, :, :]),  # dPz/dy - dPy/dz
            (pot_x[:, :-1, :, :-1, :] - pot_x[:, 1:, :, :-1, :])
            - (pot_z[:, :-1, :, :-1, :] - pot_z[:, :-1, :, 1:, :]),  # dPx/dz - dPz/dx
            (pot_y[:, :, :-1, :-1, :] - pot_y[:, :, :-1, 1:, :])
            - (pot_x[:, :, :-1, :-1, :] - pot_x[:, :, 1:, :-1, :]),  # dPy/dx - dPx/dy
        ]
        ...
        with self.warp_renderer.profiler.sample("velocity to component shape"):
            ...
            vel_sampled.append(
                ...
                if component == "X"
                    ...
                    self.warp_renderer.resample_grid3D_offset(
                        self.x,
                        offsets=[
                            (
                                [offset_coord_from, offset_coord_to, 0.0]
                                if component == "Y"
                                else [offset_coord_from, 0.0, offset_coord_to]
                            ),
                        ],
                        target_shape=self.y_shape if component == "Y" else self.z_shape,
                    ),
                    1,
                )
            )
            ...
            vel_sampled.append(
                ...
                if component == "Y"
                    ...
                    self.warp_renderer.resample_grid3D_offset(
                        ...
                        offsets=[
                            (
                                [offset_coord_to, offset_coord_from, 0.0]
                                if component == "X"
                                else [0.0, offset_coord_from, offset_coord_to]
                            ),
                        ],
                        target_shape=self.x_shape if component == "X" else self.z_shape,
                    ),
                    1,
                )
            )
                    ...
                    self.warp_renderer.resample_grid3D_offset(
                        ...
                        offsets=[
                            (
                                ...
                                if component == "X"
                                else [0.0, offset_coord_to, offset_coord_from]
                            ),
                        ],
                        target_shape=self.x_shape if component == "X" else self.y_shape,
                    ),
                    1,
                )
            )
    ...
    def stats(self, world_scale=[1, 1, 1], mask=None, state=None, **warp_kwargs):
        ...
        if mask is not None:
            mask_x = tf.greater(
                self.scale_renderer.resample_grid3D_aligned(mask, self.x_shape, align_x="stagger_output"), 0.5
            )
            x = tf.boolean_mask(x, mask_x)
        y = self.y
        if mask is not None:
            mask_y = tf.greater(
                self.scale_renderer.resample_grid3D_aligned(mask, self.y_shape, align_y="stagger_output"), 0.5
            )
            y = tf.boolean_mask(y, mask_y)
        z = self.z
        if mask is not None:
            mask_z = tf.greater(
                self.scale_renderer.resample_grid3D_aligned(mask, self.z_shape, align_z="stagger_output"), 0.5
            )
        ...
        if mask is not None:
            divergence = tf.boolean_mask(divergence, mask)
        ...
        if state is not None and state.prev is not None and state.prev.velocity is not None:
            ...
            stats["warp_angleCM_rad"] = tf_tensor_stats(
                tf.boolean_mask(warp_CangleRad, vel_CangleRad_mask), as_dict=True
            )
            del warp_CangleRad
    ...
    def __add__(self, other):
        if isinstance(other, VelocityGrid):
            if self.centered_shape != other.centered_shape:
                raise ValueError(
                    "VelocityGrids of shape %s and %s are not compatible" % (self.centered_shape, other.centered_shape)
                )
            return VelocityGrid(
                self.centered_shape,
                x=self.x + other.x,
        ...
        if isinstance(other, (np.ndarray, tf.Tensor, tf.Variable)):
            ...
            if self.centered_shape != spacial_shape_list(other) or other_shape[0] != 1 or other_shape[-1] != 3:
                raise ValueError(
                    "VelocityGrid of shape %s is not compatible with tensor of shape %s are not compatible"
                    % (self.centered_shape, spacial_shape_list(other))
                )
...
class State:
    ...
    def set_base_target_cameras_MS(self, cameras):
        assert cameras is None or (
            ...
            and all(
                isinstance(k, int) and isinstance(v, list) and all(isinstance(c, Camera) for c in v)
    ...
    def targets_raw_MS(self, scale):
        return (
            self.base_targets_raw.get_base_images_of_views_MS(scale, self.target_mask)
            if isinstance(self.base_targets_raw, ImageSetMS)
            else self.base_targets_raw.get_images_of_views_MS(scale, self.target_mask)
        )
    ...
    def targets_MS(self, scale):
        return (
            self.base_targets.get_base_images_of_views_MS(scale, self.target_mask)
            if isinstance(self.base_targets, ImageSetMS)
            else self.base_targets.get_images_of_views_MS(scale, self.target_mask)
        )
    ...
    def bkgs_MS(self, scale):
        return (
            self.base_bkgs.get_base_images_of_views_MS(scale, self.target_mask)
            if isinstance(self.base_bkgs, ImageSetMS)
            else self.base_bkgs.get_images_of_views_MS(scale, self.target_mask)
        )
    ...
    def masks_MS(self, scale):
        return (
            self.base_masks.get_base_images_of_views_MS(scale, self.target_mask)
            if isinstance(self.base_masks, ImageSetMS)
            else self.base_masks.get_images_of_views_MS(scale, self.target_mask)
        )
    ...
    def from_file(
        ...
        warp_renderer=None,
        device=None,
        density_filename="density.npz",
        velocity_filename="velocity.npz",
    ):
        density = DensityGrid.from_file(
            os.path.join(path, density_filename), as_var=as_var, scale_renderer=scale_renderer, device=device
        )
        # density = np.load(os.path.join(path, 'density.npz'), allow_pickle=True)['arr_0'].item(0).numpy() #for accidentally pickled tf.Variable
        velocity = VelocityGrid.from_file(
            os.path.join(path, velocity_filename),
            as_var=as_var,
            boundary=boundary,
            scale_renderer=scale_renderer,
            warp_renderer=warp_renderer,
    ...
    def from_scalarFlow_file(
        ...
        density = DensityGrid.from_scalarFlow_file(
            density_path, as_var=as_var, scale_renderer=scale_renderer, device=device
        )
        # density = np.load(os.path.join(path, 'density.npz'), allow_pickle=True)['arr_0'].item(0).numpy() #for accidentally pickled tf.Variable
        velocity = VelocityGrid.from_scalarFlow_file(
            velocity_path,
            as_var=as_var,
            boundary=boundary,
            scale_renderer=scale_renderer,
    ...
    def copy(self, as_var=None, device=None):
        s = State(
            self.density.copy(as_var=as_var, device=device),
            self.velocity.copy(as_var=as_var, device=device),
            frame=self.frame,
            transform=self.transform,
            targets_raw=self.base_targets_raw,
    ...
    def render_density(self, render_ctx, custom_ops=None, keep_SDF_positions=False, super_sampling=1):
        ...
        if render_ctx.render_SDF:
            if keep_SDF_positions:
                imgs, pos = render_ctx.dens_renderer.render_SDF(
                    self.get_density_transform(),
                    light_list=render_ctx.lights,
                    camera_list=self.target_cameras,
                    cut_alpha=False,
                    monochrome=render_ctx.monochrome,
                ...
                imgs = tf.stack(
                    render_ctx.dens_renderer.render_SDF(
                        self.get_density_transform(),
                        light_list=render_ctx.lights,
                        camera_list=self.target_cameras,
                        cut_alpha=False,
            ...
            if not super_sampling == 1:
                raise NotImplementedError
            imgs = tf.stack(
                render_ctx.dens_renderer.render_density(
                    self.get_density_transform(),
                    light_list=render_ctx.lights,
                    camera_list=self.target_cameras,
                    cut_alpha=False,
    ...
    def render_density_MS_stack(
        self, render_ctx, scale_shapes=None, custom_ops=None, keep_SDF_positions=False, super_sampling=1
    ):
        ...
        for scale, shape in scale_shapes.items():
            if render_ctx.render_SDF:
                if keep_SDF_positions:
                    imgs, pos = render_ctx.dens_renderer.render_SDF(
                        self.get_density_transform_MS(scale),
                        light_list=render_ctx.lights,
                        camera_list=self.target_cameras_MS(scale),
                        cut_alpha=False,
                        monochrome=render_ctx.monochrome,
                        custom_ops=custom_ops,
                        output_positions=True,
    ...
    def density_advected(self, dt=1.0, order=1, clamp="NONE"):
        return self.density.warped(
            self.velocity, order=order, dt=dt, clamp=clamp
        )  # self.velocity.warp(self.density, scale_renderer)
    ...
    def get_output_variables(self, centered=True, staggered=True, include_MS=False, include_residual=False):
        ...
        if self.has_density:
            var_dict.update(
                self.density.get_output_variables(include_MS=include_MS, include_residual=include_residual)
            )
        if self.has_velocity:
            var_dict.update(
                self.velocity.get_output_variables(
                    centered=centered, staggered=staggered, include_MS=include_MS, include_residual=include_residual
                )
            )
        return var_dict
    ...
    def stats(self, vel_scale=[1, 1, 1], mask=None, render_ctx=None, **warp_kwargs):
        ...
        if render_ctx is not None and getattr(self, "target_cameras", None) is not None:
            ...
            if getattr(self, "targets_raw") is not None and getattr(self, "bkgs") is not None:
                target_stats["SE_raw"] = tf_tensor_stats(
                    tf.math.squared_difference(self.images + self.bkgs * self.t, self.targets_raw), as_dict=True
                )
            if getattr(self, "targets") is not None:
                target_stats["SE"] = tf_tensor_stats(
                    tf.math.squared_difference(self.images, self.targets), as_dict=True
                )
        return (
            self.density.stats(mask=mask, state=self, **warp_kwargs),
            self.velocity.stats(vel_scale, mask=mask, state=self, **warp_kwargs),
            target_stats,
        )
    ...
    def stats_target(self, vel_scale=[1, 1, 1], mask=None, **warp_kwargs):
        return self.density_target.stats(mask=mask, state=self, **warp_kwargs) if self.has_density_target else None, (
            self.velocity_target.stats(vel_scale, mask=mask, state=self, **warp_kwargs)
            if self.has_velocity_target
            else None
        )
    ...
    def save(self, path, suffix=None):
        ...
        if self.has_density_proxy:
            self.density_proxy.save(
                os.path.join(path, "density_proxy.npz" if suffix is None else "density_proxy_" + suffix + ".npz")
            )
        if self.has_density_target:
            self.density_target.save(
                os.path.join(path, "density_target.npz" if suffix is None else "density_target_" + suffix + ".npz")
            )
    ...
    def clear_cache(self):
        ...
        self._SDF_positions_MS = {}
        if self.has_density:
            self.density.clear_cache()
        if self.has_density_proxy:
            self.density_proxy.clear_cache()
        if self.has_velocity:
            self.velocity.clear_cache()

    @property
    def is_MS(self):
        return ((not self.has_density) or self.density.is_MS) and ((not self.has_velocity) or self.velocity.is_MS)
...
class Sequence:
    ...
    def from_file(
        ...
        density_filename="density.npz",
        velocity_filename="velocity.npz",
        frame_callback=lambda idx, frame: None,
    ):
        # raise NotImplementedError()
        sequence = []
        prev = None
    ...
    def from_scalarFlow_file(
        ...
        for idx, frame in enumerate(frames):
            ...
            state = State.from_scalarFlow_file(
                density_path,
                velocity_path,
                frame=frame,
                transform=transform,
        ...
        if clamp is None or clamp.upper() not in ["LOCAL", "GLOBAL"]:
            for i in range(1, len(self)):
                ...
                if print_progress is not None:
                    print_progress.update(desc="Advect density {: 3d}/{: 3d}".format(i, len(self) - 1))
