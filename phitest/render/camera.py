...
from .transform import GridTransform, MatrixTransform
...
class Camera(object):
    ...
    def __init__(
        ...
        assert isinstance(grid_transform, MatrixTransform)
    ...
    # @classmethod
    # def from_eye_lookat(cls, eye, lookat, up, resolution, depth, topRightBottomLeft=[1,1,-1,-1], fov=None, aspect=1, perspective=True):
    #
    @classmethod
    def from_dict(cls, d):
    ...
    def view_transform_inverse(self, with_jitter=True):
        if self.jitter is not None and with_jitter:
            ...
            view_transform = GridTransform(
                self.transform.grid_size, translation=jitter_position, parent=self.transform
            )
        else:
    ...
    def frustum(self):
        return np.asarray(
            [self.clip[0], self.clip[1], self.trbl[3], self.trbl[1], self.trbl[0], self.trbl[2]], dtype=np.float32
        )

    # aspect: width/height if not vertical, else height/width
    def set_fov(self, fov, aspect=1, vertical=False):
        half_h = np.tan(fov / 2.0 * np.pi / 180.0) * self.clip[0]
    ...
    def copy_clipped_to_world_coords(
        self, coordinate_list, preserve_aspect=True, pad=0.0, clamp_to_original=True, preserve_clip=False
    ):
        """
        ...
        if len(coordinate_list) == 0:
            raise ValueError("Empty coordinate_list")
        # raise NotImplementedError
        # get x,y in view space in near plane
        view = MatrixTransform(self.view_matrix())
        pos_view = []
    ...
    def copy_with_frustum_crop(self, coordinate_list, pad=0):  # , preserve_aspect=True
        ...
        img_pad = [
            (offset_y, self.transform.grid_size[1] - grid_y - offset_y),
            (offset_x, self.transform.grid_size[2] - grid_x - offset_x),
            (0, 0),
        ]
